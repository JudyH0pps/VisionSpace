      ...
      <v-tooltip top>
        <template v-slot:activator="{ on, attrs }">
          <v-btn  v-on="on" v-bind="attrs" class="mx-2" color="white" @click.stop="drawer_method(4)">
            <v-icon v-if="drawer == 4" color="blue">mdi-history</v-icon><v-icon v-else>mdi-history</v-icon>
          </v-btn>
        </template>
        <span>History</span>
      </v-tooltip>
...
export default {
  ...
  methods: {
    ...
    showhistory() {
      ...
      axios.get(SERVER.URL + '/api/v1/board/' + this.$route.params.code + '/history/', config)
      .then((res) => {
        console.log(res)
